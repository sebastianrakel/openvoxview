...
const unchanged = computed(() => {
  return nodes.value.filter(s => s.latest_report_status == 'unchanged').length
})
...
<template>
  <q-page padding>
    <div class="row q-pa-sm">
      ...
      />
      <DashboardItem v-model="population" caption="Population" :title_color="q.dark ? 'text-white' : ''"/>
      <DashboardItem v-model="resources" caption="Resources managed"  :title_color="q.dark ? 'text-white' : ''"/>
      <DashboardItem v-model="avg_resources_per_node" caption="Avg. resources/node"  :title_color="q.dark ? 'text-white' : ''"/>
    </div>
    <div class="row">
      <NodeTable class="q-ma-md col" v-model:nodes="nodesNotEqualUnchaged" disable_pagination />
    </div>
  </q-page>
</template>
